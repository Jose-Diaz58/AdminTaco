...
export function GestionMesas() {
  ...
  const { mesas, obtenerMesas, crearMesa, eliminarMesa, asignarMesa, liberarMesa } = useMesas()
      ...
      <div className="bg-white rounded-xl shadow-lg flex flex-col h-[calc(100vh-160px)] overflow-hidden border border-gray-100">
        ...
        <div className="bg-linear-to-r from-orange-500 to-orange-600 text-white p-4 md:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <h2 className="text-xl md:text-2xl font-bold">Gestión de Mesas</h2>
          ...
          <div className="grid grid-cols-2 gap-3 mt-4">
            ...
            <div className="bg-orange-400 bg-opacity-40 rounded-lg p-3">
              ...
              <p className="text-2xl font-bold">
                {/* CORRECCIÓN: Validamos que cuente correctamente "Ocupado" o "Ocupada" según guarde tu backend */}
                {mesas.filter(mesa => mesa.estado === "Ocupado" || mesa.estado === "Ocupada").length}
              </p>
            </div>
          </div>
        </div>

        {/* Grid inferior de renderizado de mesas */}
